import { Collection, Field, FieldTypes, FlamestoreModule, TriggerMap } from "../../../type";
...
function triggerGenerator(
  ...
  if (isTypeReference(field) && field.path.syncFields) {
    const colNameToSyncFrom = field.path.collection;
    field.path.syncFields.forEach(syncFieldName => {

      triggerMap[collectionName].createTrigger.dependencyPromises[`ref${colNameToSyncFrom}Data`] =
        { promise: `data.${fieldName}.reference.get()`, collection: colNameToSyncFrom };
      ...
      triggerMap[colNameToSyncFrom].updateTrigger.addResultPromise(
        ...
        '${fieldName}',
        change.after.ref,
        ${collectionName}${getPascalCollectionName(field.path.collection)}Data
        )`
      );
    });
  }
